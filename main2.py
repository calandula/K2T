...
def get_keywords(keywords, context_keyword, enc_dict, tokenizer, mode):
    ...
    if context_keyword is not None:
        context_keyword_enc = enc_dict[context_keyword]
        context_keyword_gpt = {tokenizer.encode(context_keyword)[0]: context_keyword}
        return keywords_enc, keywords_gpt, context_keyword_enc, context_keyword_gpt
...
def get_sim(keywords_enc, keywords_gpt, context_keyword_enc, context_keyword_gpt, converter_table, guarantee, mode, only_max):

    if context_keyword_enc is not None:
        context_sims = cosine_similarity(np.reshape(context_keyword_enc, (1, -1)), converter_table)
...
def sample_sentence(text, this_sequence, tokenizer, model, keywords, context_keyword, enc_dict, guide_probs, converter_table, weight, guide=False, prev_proba=1, top_k=0, top_p=0.9, temperature=1., only_max=False, mode='max', guarantee=False, time=0, T_time=1, det_BS=False, ith=0, guide_context=False, context_weight=1.):
    ...
    if guide_context:
        keywords_enc, keywords_gpt, context_keyword_enc, context_keyword_gpt = get_keywords(keywords, context_keyword, enc_dict, tokenizer, mode)
    else:
        keywords_enc, keywords_gpt = get_keywords(keywords, context_keyword, enc_dict, tokenizer, mode)
    ...
    if keywords_enc and guide:
        if guide_context:
            sim, context_sims = get_sim(keywords_enc, keywords_gpt, context_keyword_enc, context_keyword_gpt, converter_table, guarantee, mode, only_max)
            context_sims = context_sims[0]
        else:
            sim = get_sim(keywords_enc, keywords_gpt, None, None, converter_table, guarantee, mode, only_max)
...
###################################
    ...
    context_keyword_enc = None
    context_keyword = None
    if guide_context:
        in_text, keywords, context_keyword = keyword_set
        context_keyword_enc = enc_dict[context_keyword]
    else:
    ...
    if guide_context:
        print(f"Prompt is: {in_text}"
              f"Keyword/s is/are: {keywords}"
              f"Context_keyword is {context_keyword}")
    else:
        print(f"Prompt is: {in_text}"
              f"Keyword/s is/are: {keywords}")
    print("#keywords: ", number_keywords)
...
########################################################################################################
    ...
    for b in range(number_of_beams):
        if guide_context:
            text_file_sentences.write(context_keyword)
...
def get_keywordsets(task, folder_name, file_name, guide_context=False):
    ...
    if task == 'key2article':
        ...
        for filename in os.listdir(folder_name):
            if filename.endswith('txt'):
                ...
                if guide_context:
                    keywords = list(lines[2].strip().split(", ")[1:])
                    context_keyword = list(lines[2].strip().split(", ")[0])
                    keyword_sets.append((' '.join(in_text), keywords, context_keyword))
                else:
        ...
        if task != 'input':
            ...
            lines = file1.readlines()
            if guide_context:
                keyword_sets = [(in_text, list(line.strip().split(", ")[1:]), line.strip().split(", ")[0]) for line in lines]
            else:
...
if __name__ == '__main__':
    ...
    keyword_sets = get_keywordsets(args.task, folder_name, file_name, guide_context=args.guide_context)
