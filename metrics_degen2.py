...
def word_sentence_similarity(sentence, enc_dict, context="soccer"):
    try:
        context_enc = enc_dict[context]
        table = str.maketrans('', '', string.punctuation)
        sentence.replace(' ', '')
        sentence = sentence.split()
        stripped = [w.translate(table) for w in sentence]
        guide_word_stems = [porter.stem(s.lower()) for s in stripped]
        sentence_enc = [enc_dict[w] for w in guide_word_stems if w in enc_dict.key_to_index and w not in stopwords.words()]
        cos_sims = [cosine_similarity([se], [context_enc]) for se in sentence_enc]
...
def main():
    ...
    with open(f'./gensim-data/glove-wiki-gigaword-300.pkl', 'rb') as f:
    ...
    with open(args.file, 'r') as file1:
        ...
        ppl = np.zeros(args.n_sample)
        cos_similarity = np.zeros(args.n_sample)
        suc_rate = np.zeros(args.n_sample)
        i = 0
        text = ''
        next_yes = False
        next_no = False
        ...
        for line in tqdm(lines):
            ...
            if "Success_rate:" in line:
                ...
                cos_similarity[i] = word_sentence_similarity(stripped, enc_dict)
                # Mean success rate
                suc_rate[i] = float(line.split(':')[1].strip())
                # Distinct n:
    ...
    text_file.write(f"Mean Success Rate: {np.mean(suc_rate)},   Mean cosine similarity: {np.mean(cos_similarity)}")
    """
    #############
    # Self-BLUE:
